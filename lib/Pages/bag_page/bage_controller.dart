...
class BageController extends GetxController{
  ...
  addBag(BuildContext context,PruductResponseModel itemBag){
    ...
    if(!addList){
      ...
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: myText("Item Add to Bag.",),duration: Duration(seconds: 2),
      )
      );
    }else{
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: myText("Sudah ada."),duration: Duration(seconds: 2),));
    }
